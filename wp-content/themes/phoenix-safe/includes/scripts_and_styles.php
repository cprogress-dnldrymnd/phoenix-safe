...
function ciyashop_scripts() {
	...
	$style_url = set_url_scheme( $upload['baseurl'] . '/ciyashop' );
	...
	$select2_js_src  = get_parent_theme_file_uri( '/js/library/select2/select2.min.js' );
	...
	wp_enqueue_script( 'popper', get_parent_theme_file_uri( '/js/library/popper.min.js' ), array(), '1.14.3', true );

	// Scripts.
	if ( is_rtl() ) {
		wp_enqueue_script( 'bootstrap', get_parent_theme_file_uri( '/js/library/bootstrap/bootstrap-rtl.min.js' ), array( 'jquery' ), '4.0.0', true );
	} else {
		wp_enqueue_script( 'bootstrap', get_parent_theme_file_uri( '/js/library/bootstrap/bootstrap.min.js' ), array( 'jquery' ), '4.1.1', true );
	}
	...
	wp_enqueue_script( 'owl-carousel', get_parent_theme_file_uri( '/js/library/owl-carousel.min.js' ), array(), '2.3.4', true );
	wp_enqueue_script( 'select2', $select2_js_src, array(), '4.0.6', true );
	wp_enqueue_script( 'jquery.countdown', get_parent_theme_file_uri( '/js/library/countdown.min.js' ), array(), '2.2.0', true );
	wp_enqueue_script( 'nanoscroller-js', get_parent_theme_file_uri( '/js/library/jquery.nanoscroller.min.js' ), array(), '0.8.7', true );
	wp_enqueue_script( 'slick-min-js', get_parent_theme_file_uri( '/js/library/slick.min.js' ), array(), '1.0.10', true );
	wp_enqueue_script( 'shuffle-js', get_parent_theme_file_uri( '/js/library/shuffle.min.js' ), array(), '3.5.2', true );
	wp_enqueue_script( 'slicknav', get_parent_theme_file_uri( '/js/library/slicknav.min.js' ), array(), '1.0.10', true );
	wp_enqueue_script( 'magnific-popup', get_parent_theme_file_uri( '/js/library/magnific-popup.min.js' ), array( 'jquery' ), '1.1.0', true );
	wp_enqueue_script( 'js-cookie', get_parent_theme_file_uri( '/js/library/cookie.min.js' ), array( 'jquery' ), '2.1.4', true );
	...
	if ( isset( $ciyashop_options['enable_lazyload'] ) && $ciyashop_options['enable_lazyload'] ) {
		wp_enqueue_script( 'ciyashop-jquery-lazy', get_parent_theme_file_uri( '/js/library/jquery.lazy/jquery.lazy.min.js' ), array( 'jquery' ), '1.7.9', true );
		wp_enqueue_script( 'ciyashop-jquery-lazy-plugins', get_parent_theme_file_uri( '/js/library/jquery.lazy/jquery.lazy.plugins.min.js' ), array( 'jquery', 'ciyashop-jquery-lazy' ), '1.4', true );
	}

	wp_register_script( 'ciyashop-main_js', get_parent_theme_file_uri( '/js/frontend/main' . $suffix . '.js' ), array( 'jquery' ), THEME_VERSION, true );

	if ( class_exists( 'WooCommerce' ) && is_product() ) {

		wp_register_script( 'ciyashop-single-product', get_parent_theme_file_uri( '/js/frontend/ciyashop-single-product' . $suffix . '.js' ), array( 'jquery', 'ciyashop-main_js', 'zoom' ), '3.5.2', true );

		$zoom_data = array();
		$zoom      = true;
		if ( isset( $ciyashop_options[ 'product_zoom_magnify' ] ) && $ciyashop_options[ 'product_zoom_magnify' ] ) {
			$zoom_data['magnify'] = $ciyashop_options[ 'product_zoom_magnify' ];
		}

		if ( isset( $ciyashop_options[ 'product_zoom' ] ) ) {
			$zoom = ( bool ) $ciyashop_options[ 'product_zoom' ];
		}

		$ciyashop_wc_single_product_params = array(
			'zoom_enabled' => apply_filters( 'ciyashop_woocommerce_single_product_zoom_enabled', $zoom ),
			'zoom_options' => apply_filters( 'ciyashop_woocommerce_single_product_zoom_options', $zoom_data ),
		);

		/**
		 * Localizes a single product script with data for a JavaScript variable.
		 *
		 * @param array    $l10n      Array of data.
		 *
		 * @visible true
		 */
		$ciyashop_wc_single_product_params = apply_filters( 'ciyashop_wc_single_product_params', $ciyashop_wc_single_product_params );

		wp_localize_script( 'ciyashop-single-product', 'ciyashop_wc_single_product_params', $ciyashop_wc_single_product_params );

		wp_enqueue_script( 'ciyashop-single-product' );

		wp_enqueue_script( 'ciyashop-add-to-cart-variation', get_parent_theme_file_uri( '/js/frontend/ciyashop-add-to-cart-variation' . $suffix . '.js' ), array( 'jquery', 'ciyashop-main_js' ), '3.5.2', true );
	}
	...
	$ajax_add_to_cart = isset( $ciyashop_options['ajax_add_to_cart'] ) ? $ciyashop_options['ajax_add_to_cart'] : false;
	if ( $ajax_add_to_cart ) {
		// Enqueued script ajax add to cart.
...
function ciyashop_admin_enqueue_scripts( $hook ) {
	...
	// Javascript.
	wp_register_script( 'ciyashop_admin_js', get_parent_theme_file_uri( '/js/admin/admin' . $suffix . '.js' ), array( 'jquery' ), THEME_VERSION, true );
	...
	wp_enqueue_script( 'ciyashop_icon_picker', get_parent_theme_file_uri( '/js/library/jquery.fonticonpicker.min.js' ), array( 'jquery' ), '3.5.2', true );
	...
	if ( 'toplevel_page_header-builder' === $hook || 'header-builder_page_header-layout' === $hook ) {
		...
		wp_enqueue_script( 'select2', get_parent_theme_file_uri( '/js/library/select2/select2.min.js' ), array(), '3.5.2', true );
	}
...
// Remove the elementor Fontawesome
