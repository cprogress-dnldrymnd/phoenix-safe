...
/**
 * Save Color Customizer when customizer option on preview
 */
function ciyashop_action_customize_preview_init() {
	global $ciyashop_options;
	ciyashop_sample_data_theme_option( $ciyashop_options );
};
add_action( 'customize_preview_init', 'ciyashop_action_customize_preview_init' );

/**
 * Save Color Customizer when customizer option on saved
 */
function ciyashop_action_customize_save_after( $data ) {
	global $ciyashop_options;

	$ciyashop_options_data = json_decode( wp_unslash( $_POST['customized'] ), true );
	$primary_color         = isset( $ciyashop_options_data['ciyashop_options[primary_color]'] ) ? $ciyashop_options_data['ciyashop_options[primary_color]'] : '';
	$secondary_color       = isset( $ciyashop_options_data['ciyashop_options[secondary_color]'] ) ? $ciyashop_options_data['ciyashop_options[secondary_color]'] : '';
	$tertiary_color        = isset( $ciyashop_options_data['ciyashop_options[tertiary_color]'] ) ? $ciyashop_options_data['ciyashop_options[tertiary_color]'] : '';
	$header_schema         = '';
...
add_action( 'customize_save_after', 'ciyashop_action_customize_save_after' );
...
#site-navigation-sticky-mobile .slicknav_nav .current-menu-item > a .slicknav_arrow,
  ...
  .woocommerce [class*=product-hover-style-grocery-] .product-info .product-action .button { background: ' . $primary_color . '; }
...
.woocommerce .products [class*=product-hover-style-grocery-] .product-info .price .amount { color: ' . $primary_color . '; }
		...
		foreach ( $header_schema as $key => $value ) {
			...
			if ( 'menu_color_schema' === (string) $key ) {
				...
				if ( ! empty( $link_bg_color ) ) {
					$header_customize .= '
						.header-' . $key . ' #site-navigation-sticky-mobile > span,
...
function ciyashop_get_custom_header_schema( $header_id = '' ) {
	...
	if ( ! empty( $topbar_configuration ) && is_array( $topbar_configuration ) ) {
		foreach ( $topbar_configuration as $value ) {
			$topbar_config[ $value['name'] ] = $value['value'];
		}
	}

	if ( ! empty( $main_configuration ) && is_array( $main_configuration ) ) {
		foreach ( $main_configuration as $value ) {
			$main_config[ $value['name'] ] = $value['value'];
		}
	}

	if ( ! empty( $bottom_configuration ) && is_array( $bottom_configuration ) ) {
