...
function ciyashop_vc_templates_enqueue_scripts() {
	...
	wp_register_script( 'jquery-lazy', get_parent_theme_file_uri( '/js/library/jquery.lazy/jquery.lazy.min.js' ), array(), '1.7.9', true );

	// VC Templates Backend.
	if ( ! vc_is_frontend_editor() && vc_check_post_type( $post_type ) ) {

		wp_register_script( 'ciyashop_vc_templates_js', get_parent_theme_file_uri( '/js/admin/vc-templates' . $suffix . '.js' ), array( 'ciyashop_admin_js', 'jquery-lazy' ), WPB_VC_VERSION, true );
		wp_enqueue_script( 'ciyashop_vc_templates_js' );
	}

	// VC Templates Frontend.
	if ( vc_is_frontend_editor() ) {
		wp_register_script( 'ciyashop_vc_templates_js', get_parent_theme_file_uri( '/js/admin/vc-templates' . $suffix . '.js' ), array( 'vc-frontend-editor-min-js', 'jquery-lazy' ), WPB_VC_VERSION, true );
		wp_enqueue_script( 'ciyashop_vc_templates_js' );
	}
}
