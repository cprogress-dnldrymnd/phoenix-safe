...
(function($){
	...
	jQuery( document ).ready(function() {
		...
		*********************************************/
		...
		if ( $( 'body' ).hasClass( 'cs-filter-with-ajax' ) ) {
			...
			$( document ).on( 'click', '.pgs-woocommerce-widget-stock-status-widget .pgs-woocommerce-widget-stock-status-widget-item a, .woocommerce-widget-layered-nav-list .woocommerce-widget-layered-nav-list__item a, .pgs-woocommerce-widget-layered-nav-list .woocommerce-widget-layered-nav-list__item a, .ciyashop-clear-filters, .widget_layered_nav_filters ul li a, .widget_product_categories ul li a, .widget_rating_filter ul li a, .woocommerce-categories-slider .item a, .ciyashop-products-per-page .per-page-variation', function (e) {
				e.preventDefault(e);
		...
		*************************/
		...
		window.onscroll = function() { ciyashop_sticky_menu() };
		...
		function ciyashop_sticky_menu() {
			if ( header_sticky_el ) {
				if(
					...
					if ( ciyashop_l10n.sticky_header == '1' && ciyashop_l10n.device_type == 'desktop' && ciyashop_l10n.sticky_header_mobile == '0' && $(window).width() <= 992 ) {
						return;
					}
					...
					if ( $(window).width() > 992 ) {
						$( header_sticky_el ).css( 'opacity',0 ).css( 'display', 'block' );
					}

					var cur_position         = jQuery(window).scrollTop();
					var sticky_header        = masthead.offsetTop + masthead.offsetHeight;
					var sticky_header_height = $( '.header-sticky-inner' ).outerHeight();

					header_sticky_el.style.height = sticky_header_height + 'px';

					if ( cur_position > sticky_header ) {
						header_sticky_el.style.position = 'fixed';
						header_sticky_el.style.top      = sticky_top + 'px';
						header_sticky_el.style.opacity  = 1;
						header_sticky_el.style.display  = 'block';
						header_sticky_el.style.width    = $(window).width() + 'px';
						header_sticky_el.style.height   = sticky_header_height + 'px';
						header_sticky_el.classList.add( 'is-sticky' );
					} else {
						header_sticky_el.style.position = null;
						header_sticky_el.style.top      = null;

						if ( $(window).width() > 992 ) {
							header_sticky_el.style.opacity  = 0;
						}

						header_sticky_el.style.display  = 'block';
						header_sticky_el.style.height   = sticky_header_height + 'px';
						header_sticky_el.style.width    = null;
						header_sticky_el.classList.remove( 'is-sticky' );
					}
				}
		...
		*************************/
		...
		if ( ! $('#site-navigation-sticky-mobile').hasClass( 'sidemnu' ) ) {
			...
			$( '.cs-menu-nav .menu-item-has-children > a' ).append( '<span class="sub-menu-icon sub-menu-open-icon"></span>' );
			$( document ).on( 'click', '.sub-menu-icon', function( e ) {
				e.preventDefault();

				if ( $( this ).closest( '.menu-item' ).hasClass( 'menu-opend' ) ) {
					$( this ).closest( '.menu-item' ).removeClass( 'menu-opend' ).find( '> ul' ).slideUp( 300 );
					$( this ).removeClass( 'sub-menu-close-icon' ).addClass( 'sub-menu-open-icon' );
				} else {
					$( this ).closest( '.menu-item' ).addClass( 'menu-opend' ).find( '> ul' ).slideDown( 300 );
			...
			$( document ).on( 'click', '.ciyashop-sidemenu-overlay', function ( e ) {
				...
				$( '.ciyashop-side-menu-mobile-container' ).removeClass( 'side-menu-open' );
			});

			$( document ).on( 'click', '.ciyashop-mobile-nav-tabs-container .mobile-tab', function( event ) {

				$( '.ciyashop-mobile-nav-tabs-container .mobile-tab' ).removeClass( 'active' );
			...
			$( document ).on( 'click', '#site-navigation-sticky-mobile', function( event ) {
				if ( $( '.ciyashop-side-menu-mobile-container' ).hasClass( 'side-menu-open' ) ) {
					$( '.ciyashop-side-menu-mobile-container' ).removeClass( 'side-menu-open' );
				} else {
					$( '.ciyashop-side-menu-mobile-container' ).addClass( 'side-menu-open' );
				}

				if ( $( '.ciyashop-sidemenu-overlay' ).hasClass( 'side-menu-open' ) ) {
					$( '.ciyashop-sidemenu-overlay' ).removeClass( 'side-menu-open' );
				} else {
					$( '.ciyashop-sidemenu-overlay' ).addClass( 'side-menu-open' );
				}
			});
		}
		...
		****************************/
		...
		$( document.body ).on( 'cs_shop_ajax_after_content_load', function ( event, url, pagination ) {
			cs_product_infinite_scroll();
		});
				...
				if( $('.product-infinite_scroll').length > 0 ){
					if( $('.product-more-button a').length > 0 ) {
						...
						if ( scrollPosition > scrollHeight + scrollOuterHeight ) {
							setTimeout(function () {
								if( total == cs_product_infinite_scroll_counter ){
									if(!load_more_button.hasClass('content-loading')){
										cs_product_infinite_scroll_counter++;
		...
		*******************************/
		...
		$(document).scroll(function () {
			if ( $( '.product_title.entry-title' ).length > 0 && $(window).width() >= 768 && $( '#woo-product-sticky-content' ).length > 0 ) {

				var header_sticky_adminbar = $('#wpadminbar');
				var windows_height         = $(window).height();
				var element_height         = $( '.product_title.entry-title' ).offset().top;
				var sticky_el              = document.getElementById( 'woo-product-sticky-content' );
				var position               = 10;
				...
				if ( current_position > element_height ) {
					...
					sticky_el.style.top = position + 'px';
					sticky_el.style.display = 'block';
					sticky_el.classList.add( 'is-sticky' );
				} else if ( (current_position < element_height ) ) {
					sticky_el.style.position = null;
					sticky_el.style.top = null;
		...
		if ( $("body.single-product .product.type-product").length > 0 && $("body.single-product .product.type-product").hasClass('product_page_style-sticky_gallery') ) {
			...
			jQuery(window).on( 'scroll', function(){
				...
				if( page_style_sticky_header_sticky_visible != page_style_sticky_header_sticky_visible_new ){
					...
					if( jQuery('#header-sticky.is-sticky').is(":visible") ){
						var product_style_sticky__sticky_header_height = jQuery('#header-sticky.is-sticky').outerHeight();
		...
		************************************************************************************/
		...
		$(document).on("click", ".swatches-select a.ciyashop-swatch", function(){
			...
			var val_id        = $(this).attr('data-cs_parent').replace(/[!"#$%&'()*+,.\/:;<=>?@[\\\]^`{|}~]/g, "\\\$&");
            ...
            $.ajax({
                ...
                success: function(response) {
					...
					if( typeof response.notices != 'undefined' && response.notices.indexOf( 'error' ) > 0 ) {
						$( '.woocommerce-notices-wrapper' ).replaceWith( response.notices );

						if ( $( '.woocommerce-error:first' ).length > 0 ) {
							var error_position = $( '.woocommerce-error:first' ).offset().top - $( '.woocommerce-error:first' ).outerHeight() - 50;
		...
		$( window ).scroll(function() {
			...
			*****************************************/
			...
			if( $('.wpb_column.sticky-top').length > 0 ){
				...
				var page_header_sticky_visible_new = jQuery('#header-sticky.is-sticky').is(":visible");

				if( page_header_sticky_visible != page_header_sticky_visible_new ){
					page_header_sticky_visible = page_header_sticky_visible_new;

					if( jQuery('#header-sticky.is-sticky').is(":visible") ){
						var sticky_header_height = jQuery('#header-sticky.is-sticky').outerHeight();
	...
	**************************************************/
	...
	$( window ).on( 'elementor/frontend/init', function () {
		...
		elementorFrontend.hooks.addAction( 'frontend/element_ready/pgscore_vertical-menu.default', cs_vertical_menu );
	...
	function cs_vertical_menu() {
		if ( jQuery( '.pgscore_v_menu__menu_wrap' ).length > 0 ) {
			jQuery( '.pgscore_v_menu__menu_wrap' ).each( function( idx, item ) {
				var v_menu_wrap     = this,
					v_menu          = jQuery( v_menu_wrap ).find( '.pgscore_v_menu__nav' ),
					v_menu_parent   = jQuery( v_menu_wrap ).parent(),
					v_menu_branding = '';

					if ( jQuery( v_menu_parent ).data( 'menu_title' ) !== undefined && $( v_menu_parent ).data( 'menu_title' ) !== '' ) {
						v_menu_branding = jQuery( v_menu_parent ).data( 'menu_title' );
					}
				...
				v_menu.slicknav({
					...
					'init': function(){
						jQuery( v_menu_parent ).find( '.slicknav_menu' ).addClass( 'slicknav_menu-wrap' ).removeClass( 'slicknav_menu' );
						jQuery( v_menu_parent ).find( '.slicknav_nav' ).addClass( 'menu' );

						if ( jQuery( v_menu_parent ).find( '.pgscore_v_menu__nav' ).hasClass( 'pgs_megamenu-enable' ) ) {
							jQuery( v_menu_parent ).find( '.slicknav_nav' ).addClass( 'pgs_megamenu-enable' );
						}
					},
					afterOpen: function( trigger ) {
