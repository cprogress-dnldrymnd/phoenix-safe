...
function pgscore_get_terms_hierarchical_list( $terms = array(), $new_array = array() ) {

	foreach ( $terms as $term ) {
		$new_array[ $term->term_id ] = $term;

		if ( ! empty( $term->children ) ) {
			$new_array = pgscore_get_terms_hierarchical_list( $term->children, $new_array );
			unset( $term->children );
		} else {
			unset( $term->children );
		}
 ...
 * @return array
 */
function pgscore_get_terms_hierarchy( $taxonomy, $parent = 0 ) {
	// only 1 taxonomy
	...
	if ( ! is_wp_error( $terms ) ) {
		...
		foreach ( $terms as $term ) {
			...
			$term->children = pgscore_get_terms_hierarchy( $taxonomy, $term->term_id );
