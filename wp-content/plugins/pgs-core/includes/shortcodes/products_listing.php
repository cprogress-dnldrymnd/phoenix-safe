...
function pgscore_shortcode_products_listing( $atts, $content = null, $shortcode_handle = '' ) {
	$default_atts = array(
		...
		'list_grid_columns'                  => 4,
		'list_grid_columns_small'            => 1,

		'list_carousel_items_xs'             => 1,
...
if ( function_exists( 'vc_map' ) && ( is_admin() || vc_is_frontend_ajax() || vc_is_frontend_editor() || vc_is_inline() ) ) {
	...
	$shortcode_fields = array(
		...
		array(
			'type'        => 'pgscore_ajax_select',
			'param_name'  => 'product_ids',
			'heading'     => esc_html__( 'Select Products', 'pgs-core' ),
			'save_always' => true,
			'admin_label' => true,
			'multiple'    => true,
		...
		array(
			'type'        => 'dropdown',
			'param_name'  => 'list_grid_columns_small',
			'heading'     => esc_html__( 'Grid Columns for small devices', 'pgs-core' ),
			'description' => esc_html__( 'Select listing grid columns.', 'pgs-core' ),
			'save_always' => true,
			'value'       => array_flip(
				array(
		...
		array(
			'type'             => 'dropdown',
			'param_name'       => 'list_carousel_items_xs',
			'heading'          => esc_html__( 'Extra Small Devices (&lt;480px )', 'pgs-core' ),
		...
		array(
			...
			'heading'          => esc_html__( 'Small Devices (&ge;480px )', 'pgs-core' ),
			'description'      => esc_html__( 'Select number of items to display at a time in small devices.', 'pgs-core' ),
			'save_always'      => true,
			'value'            => array_flip(
				array(
