...
$this->add_render_attribute( 'pgscore_testimonial_list', 'class', 'testimonials owl-carousel owl-theme owl-carousel-options' );
		...
		if ( $content ) {
			...
			<div class="item">
				<div class="client-image">
					<?php
					if ( ! $testimonials_img_src ) {
						$testimonials_img_src = get_parent_theme_file_uri( '/images/placeholder/testimonials/150x150.png' );
					}
					?>
					<div>
						<div class="author-photo">
							<?php echo '<img class="img-responsive" src="' . esc_url( $testimonials_img_src ) . '" alt="' . esc_attr( $testimonials_img_alt ) . '" />'; ?>
						</div>
					</div>
				</div>
				...
				<?php echo wp_kses( $content, array( 'p' => true ) ); ?>
			</div>
			<?php
		}
	}
